...
`GET\n${uri}\n\nhost:${S3_HOST}\nx-amz-content-sha256:${ph}\nx-amz-date:${ad}\n\nhost;x-amz-content-sha256;x-amz-date\n${ph}`;
  ...
  const sc=`${ds}/${REGION}/s3/aws4_request`;
  const st=`AWS4-HMAC-SHA256\n${ad}\n${sc}\n${ch}`;
  let kd=await hmac("AWS4"+SK,ds);
...
`AWS4-HMAC-SHA256 Credential=${AK}/${sc},SignedHeaders=host;x-amz-content-sha256;x-amz-date,Signature=${sig}`;
  return fetch(`https://${S3_HOST}${uri}`,{
    headers:{
      "Host":S3_HOST,
      "x-amz-date":ad,
      "x-amz-content-sha256":ph,
...
async function handle(req){
  ...
  if(url.pathname.includes("/provide/type")){
    try{
      ...
      return new Response(JSON.stringify({code:1,msg:"ok",list:types.map((n,i)=>({type_id:i+1,type_name:n}))}),{headers:cors});
    }catch(e){
